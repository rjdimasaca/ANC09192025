				...
				if(searchResults[HeadIndex].getValue('custrecord_syncwmloadhead_customerkey') != '5786' && searchResults[HeadIndex].getValue('custrecord_syncwmloadhead_consigneekey') != '6582') {
					...
					if(searchInvoice5) {
						for(k=0; k<searchInvoice5.length; k++){
							currInvKey = searchInvoice5[k].getValue('custbody_wmorderheaderkey');
								...
								if(currOHKeys.indexOf(LineDetails[lineKey]["OrderHeader"]) > -1){
									...
									if ((testQtyToShip == '0' || testQtyToShip == 0) && (testQtyShipped == '0' || testQtyShipped == 0) && testInvList == '') {
										...
										if(LineRec.getFieldValue('custrecord_syncwmloaddet_rollsshipped') > 0) {
											...
											Record5.setCurrentLineItemValue('item','item',lineItem);
											...
											itemPrice = validateConsigneePrice(lineItem,ValidCustomer,ValidConsignee,tempDate);
											...
											/*
											//Check Currency for Tax - currently ignored as it's set by Customer
											primaryCurrency = nlapiLookupField('customer',ValidCustomer,'currency');
											if (primaryCurrency == 1 || primaryCurrency == '1') {
												//CAD - Add Tax
...
function findInvItemFromSerialNumbers(inputNumbers,SyncWMID) {
	...
	var filters = [];
	filters[0] = new nlobjSearchFilter('inventorynumber', 'inventorynumber', 'is', LookupSerialNo);
	var columns = [];
	columns[0] = new nlobjSearchColumn('internalid');
	...
	var itemResults = nlapiSearchRecord('inventorydetail', null, filters, columns);
...
function transferInventory(adjDate, cutoffDate, customerName, SyncWMRecord, LineIdObj, searchLineResults, locationConsigneeObject) {
	...
	InvTfrRecord.setFieldValue('location', '215');
		...
		if (LineIdObj[LineId] != "undefined") {
			...
			if ((testQtyToShip == '0' || testQtyToShip == 0) && (testQtyShipped == '0' || testQtyShipped == 0) && testInvList == '') {
				...
				if (LineRec.getFieldValue('custrecord_syncwmloaddet_rollsshipped') > 0) {
					...
					for (x = 0; x < lineQty; x++) {
						TESTSTRING = TESTSTRING + 'F';
						serialNumberLookupObj = serialNumberLookup(serialNumbers[x], SyncWMID);
