	...
	function getLineItemsToPrintHtml(lineItemsToPrint)
	{
		...
		try
		{
			...
			*/
				...
				lineValue += lineValue ? '<p align="left"><barcode bar-width=".75" codetype="code128" showtext="true" value="' + lineValue + '"></barcode></p>' : "";
...
//				var lineValue = lineItemsToPrint[a].getValue(lineItemsToPrint[a].columns[3]) //.columns[3]) description line from transaction
				...
				var lineValue = lineItemsToPrint[a].getValue(lineItemsToPrint[a].columns[8])
				lineValue = clean(lineValue);
//				lineValue = "<br/>" + lineValue;
//				lineValue = "<br/>" + lineValue;
				tdValues.push({lineValue : lineValue, styleClassName : "unitofmeasure_content"});
...
//				lineValue = "<br/>" + lineValue;
//				lineValue = "<br/>" + lineValue;
				tdValues.push({lineValue : lineValue, styleClassName : "itemcount_content"});
				
				//{name: "quantity", label: "Quantity"}
				var lineValue = "" //.columns[6])
				lineValue = clean(lineValue, true);
				tdValues.push({lineValue : lineValue, styleClassName : "comments_content"});
	...
	function getLineItemsToPrint(context)
	{
		...
		transactionSearchObj = search.create({
			   ...
			   [
			      ...
			      ["taxline","is","F"],
			      "AND", 
			      ["transactionlinetype","anyof","COUNTQUANTITY"]
			   ],
			   columns:
			   transactionSearchObj_columns
			});
...
//		var formattedDate = format.parse({value:new Date(), type: format.Type.DATETIME});
    ...
    function onRequest(context)
    {
    	...
    	var templateFileId = 215827;//SB
    	var templateFileId = 215827;//PROD
    	var templateFileId = './010TMPLT_countSheet.html';//PROD
