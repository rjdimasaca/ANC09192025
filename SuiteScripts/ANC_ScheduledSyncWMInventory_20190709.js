...
/**
 * 11052019 - added validation for GRADE52
 * 1.2.1 - follows load version. added GRADE53 as NPT59CH as said by mark sullivan on skype - 04232020
 */
...
var minDate = new Date(2019, 5, 1); //June 1, 2019
var loopCount = 0;
...
function ScheduledSyncWMInventory(params) {
  ...
  try {
	...
	if(SyncWMRecord.getFieldValue('custrecord_syncwminv_wmid') != null && SyncWMRecord.getFieldValue('custrecord_syncwminv_wmid') != ""){ SearchValue = SyncWMRecord.getFieldValue('custrecord_syncwminv_wmid'); } else { SearchValue = SyncWMRecord.getFieldValue('name')};
	
	nlapiLogExecution('debug','Test Values','SyncWMID: '+SyncWMID+' SyncWMRecord: '+SearchValue);
	
    var filters = [
      ['internalid', 'noneof', [SyncWMID]], 'AND',
      ['custrecord_syncwminv_wmid', 'contains', SearchValue], 'AND',
    ...
    if (searchDuplicates) {
      ...
      var filters = [
        ...
        ['custrecord_syncwminv_syncstatus', 'anyof', [1, 2, 3, 5]], 'AND',
      ...
      if (searchResults) {
        var cappedSearchLength = Math.min(searchResults.length, 50); //Testing with 200. Need to validate exact max requirements to set to valid length.
		...
		tempDate = (tempDate < minDate) ? minDate : tempDate;
        ...
        tempOut = validateItem(itemIn[0]);//TC10ROD checks if item is valid, or in other words there is an item mapped for this

        if (tempOut != '' && tempOut != 'error') {
          itemOut.push(tempOut);
          //TC10ROD why add to the list without checking if it can really be generated only because item is mapped for SALE23?
          var numberExistsInNS = [];
          checkSerialNumItemOut = validateItem(SyncWMRecord.getFieldValue('custrecord_syncwminv_qualitystatus') + SyncWMRecord.getFieldValue('custrecord_syncwminv_prodgradekey'));
          numberExistsInNS = checkNetSuiteSerialNumbers(SyncWMRecord.getFieldValue('custrecord_syncwminv_inventoryno'), checkSerialNumItemOut);
          
          if (numberExistsInNS[0] == 'diffitem') {
              nlapiLogExecution('DEBUG', 'ExecOrder', 'B');
              throw nlapiCreateError('error', 'Serial Number ' + SyncWMRecord.getFieldValue('custrecord_syncwminv_inventoryno') + ' already exists in NetSuite for a different item grade or quality');
              ...
              if (checkExists > -1) { //if (checkExists > -1 && checkSerialNo == 'valid') {
                ...
                if (itemIDs.length > checkExists) {
                  ...
                  repeatedIDIndex = itemIDs[checkExists].indexOf(SyncWMRecord.getFieldValue('custrecord_syncwminv_inventoryno'));
                  if (repeatedIDIndex == -1) {
                    nlapiLogExecution('DEBUG', 'ExecOrder', 'F');
                    itemIDs[checkExists].push(SyncWMRecord.getFieldValue('custrecord_syncwminv_inventoryno'));
                  }
                } else {
          ...
          //TC10ROD this is done by the above code, except with condition
//          itemIDs.push([SyncWMRecord.getFieldValue('custrecord_syncwminv_inventoryno')]);
        } else {
        ...
        removeRec.setFieldValue('department', '1');
        ...
        for (z = 0; z < itemOut.length; z++) {
          if (numberExistStatus[z][0] == 'sameitem') {
            ...
            if (serialResults) {
			  ...
			  removeRec.setCurrentLineItemValue('inventory', 'department', '2');
    	...
    	nlapiLogExecution("DEBUG", "itemIn", itemIn);
        for (k = 0; k < itemIn.length; k++) {
        	nlapiLogExecution("DEBUG", "k", k);
        	nlapiLogExecution("DEBUG", "JSON.stringify(itemIn)", JSON.stringify(itemIn));
        	nlapiLogExecution("DEBUG", "numberExistStatus[k][0] != 'sameitem'", numberExistStatus[k][0] != 'sameitem');
        ...
        for (x = 0; x < cappedSearchLength; x++) {
        	nlapiLogExecution("DEBUG", "searchResults[x].getValue('internalid'), x : " + x, searchResults[x].getValue('internalid'))
        	nlapiLogExecution("DEBUG", "numberExistStatus[x][0]", numberExistStatus[x][0])
          if (numberExistStatus[x][0] != 'sameitem') {
            ...
            var updatedRecordId = nlapiSubmitField('customrecord_syncwminventory', searchResults[x].getValue('internalid'), fieldArr, valueArr); //Complete
            
        	nlapiLogExecution("DEBUG", "updatedRecordId, x : " + x, updatedRecordId)
            /*} else {
        	...
        	//TC10ROD
        	//regardless, mark the rest of the search results with success, keep the condition so that you can define specific message for when this happens
          else
          {
            //TC10ROD 03282020 only update the batchmembers as success if the current batchmember is success
            //current batchmember cannot be succeed with no or zero addLineCount
            if (addLineCount > 0) {
              
              fieldArr = ['custrecord_syncwminv_syncstatus', 'custrecord_syncwminv_errmsg', 'custrecord_syncwminv_nsid'];
              valueArr = [4, 'Created Inventory Adjustment record with Internal ID ' + RecordID + " : this is considered a success because an earlier batchmember sucessfully got the whole batch's serials captured", RecordID];
              var updatedRecordId = nlapiSubmitField('customrecord_syncwminventory', searchResults[x].getValue('internalid'), fieldArr, valueArr); //Complete
            }
        ...
        //***---*** Commented out as should be handled above ***---***
...
function findNextRecord() {
  ...
  var filters = [
	...
	//['custrecord_syncwminv_dateproduced', 'startswith', '2019-06'], 'AND',
    ['custrecord_syncwminv_retries', 'greaterthan', 0]
  ];

  var columns = [];
  columns[0] = new nlobjSearchColumn('internalid');
  columns[1] = new nlobjSearchColumn('custrecord_syncwminv_dateproduced').setSort(true); //Sorts by most recent date first
...
function validateItem(aggKey) {
  ...
  if (prodGrade == null || prodGrade == '') {
    ...
    switch (prodGrade) {
      ...
      case '52':
        return '78961'; //DYW56ST Prime - MFG (avg cost)
      case '53':
    ...
    switch (prodGrade) {
      ...
      case '52':
          return '78962'; //DYW56ST Off-Grade- MFG (avg cost)
      case '53':
    ...
    switch (prodGrade) {
      ...
      case '52':
          return '78963'; //DYW56ST Beater - MFG (avg cost)
      case '53':
